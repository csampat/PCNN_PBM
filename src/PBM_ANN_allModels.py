...
# loss_densPINN, loss3, loss4, mae_densPINN, mse_densPINN, mae4, mse4 = PINNdens_model.evaluate(normed_test_dataset, [label1,label2], verbose=0)
# test_predictions_densPINN = PINNdens_model.predict(normed_test_dataset)
...
EPOCHS = 300
# psdModel, psdHistory = densMod.build_train_psdPINN_l2_dropout(dens_normed_train_dataset, dens_train_labels1,patience_model,16,'relu',EPOCHS)

psdModel, psdHistory = densMod.build_train_psdPINN_2intLayers(dens_normed_train_dataset1, train_label2,patience_model,8,16,'relu',EPOCHS)
...
r2model_ANNPSD = densMod.calculateR2(psdPredict.flatten(),test_label2,labels2)
...
# print("Test set Loss(MSE), MAE and R^2 of ", model7 ,"is",  loss_densPINN, mae_densPINN, r2model_PINN)
...
# comparing 3 models
plt.figure()
plotObj.history_plotter_comparen_noval([history, stref_history,smaxref_history,densHistory,psdHistory], 'loss', [model1, model2, model3,model9,model10])
...
testIdx = [25, 500, 3000]
plotObj.expDataPlot_comparen([test_conv_sm,test_conv_stref,test_conv_smax,test_conv_dens],test_labels,testIdx,sieveCut,[model1,model2,model3,model9])
'''
